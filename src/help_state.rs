...
impl State for HelpState {
    fn update(&mut self, context: &mut Context) -> Option<Box<dyn State>> {
        if context.input.key_just_pressed(&Keycode::Return) {
            ...
            return Some(Box::new(StartMenuState { selected_option: 1 }))
        }
        None
    }
    ...
    fn draw(&mut self, context: &mut Context, canvas: &mut WindowCanvas) {
        ...
        let play = font.render("Movement: Arrow Keys").blended(Color::RGBA(255, 255, 255, 255)).unwrap();
        ...
        let help = font.render("Reset Puzzle: \"R\" key").blended(Color::RGBA(255, 255, 255, 255)).unwrap();
        ...
        let credits = font.render("Exit Puzzle: \"Q\" key").blended(Color::RGBA(255, 255, 255, 255)).unwrap();
